...
- (void)reload {
  _flags.isViewInvalid = YES;
  [self.model load:NSURLRequestUseProtocolCachePolicy more:NO];
}
...
- (void)refresh {
  ...
  if (!loading && !loaded && [self shouldLoad]) {
    ...
    [self.model load:NSURLRequestReloadIgnoringCacheData more:NO];

  } else if (!loading && [self shouldLoadMore]) {
    [self.model load:NSURLRequestUseProtocolCachePolicy more:YES];
